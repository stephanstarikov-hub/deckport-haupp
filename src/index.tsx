...
function Content() {
  ...
  const [page, setPage] = useState<"home" | "servers" | "subscriptions">("home");
  ...
  return <Focusable onCancel={page === "home" ? undefined : e => { e.stopPropagation(); goBack(); }} style={{ paddingBottom: 16 }}>
    ...
    {page === "home" && <>
      ...
      <PanelSection title="Public IP">
        <div>Before: {status?.before_ip ?? "—"}</div>
        <div>After: {status?.public_ip ?? "—"}</div>
        <PanelSectionRow><ButtonItem layout="below" disabled={!connected || busy} onClick={() => void run(async () => {
          const result = await api.verify();
          toaster.toast({ title: "DeckPort VPN", body: result.verified ? "Tunnel route and internet access verified" : "Tunnel verification failed. Disconnect and reconnect." });
        })}>Verify connection</ButtonItem></PanelSectionRow>
      </PanelSection>
      <PanelSection>
        <PanelSectionRow><ButtonItem layout="below" onClick={() => form()}>+ Add subscription</ButtonItem></PanelSectionRow>
    ...
    {page !== "home" && <PanelSection><PanelSectionRow><ButtonItem layout="below" onClick={goBack}>‹ Back</ButtonItem></PanelSectionRow></PanelSection>}
    {page === "servers" && <PanelSection title="Servers">
      ...
      {results.slice(0, limit).map(n => <PanelSectionRow key={n.id}><ButtonItem layout="below" disabled={busy} description={n.protocol.toUpperCase()} onClick={() => void run(async () => { await api.select(n.id); setPage("home"); })}>{n.id === status?.selected ? "✓ " : ""}{n.name}</ButtonItem></PanelSectionRow>)}
